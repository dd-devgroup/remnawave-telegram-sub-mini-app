...
| `REMNAWAVE_PANEL_ACCESS_KEY` | Optional. Shared secret used when the panel is protected by a URL-parameter gate. The app will automatically add `?<KEY>=<KEY>` to every request and send a matching `Cookie` header.                                                                                   |
...
| `AUTH_API_KEY`               | If you use "Caddy with security" or TinyAuth for Nginx addon, you can place the X-Api-Key here to apply it to requests to Remnawave Panel.                                                                                                                              |
